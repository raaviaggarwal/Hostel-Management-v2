...
if (isset($_POST['submit'])) {
    // Get input values
    $seater = $_POST['seater'];
    $roomno = trim($_POST['rmno']);  // Trim any extra spaces
    $fees = $_POST['fee'];

    // Validate that roomno is not empty
    if (empty($roomno)) {
        echo "<script>alert('Please enter a room number');</script>";
    } else {
        // Prepare SQL to check if room exists
        $sql = "SELECT room_no FROM rooms WHERE room_no=?";
        $stmt1 = $mysqli->prepare($sql);
        $stmt1->bind_param('s', $roomno);  // Bind room number as string (VARCHAR)
        $stmt1->execute();
        $stmt1->store_result(); 
        ...
        if ($row_cnt > 0) {
            ...
            $rc = $stmt->bind_param('iss', $seater, $roomno, $fees); // Bind seater as integer, room_no as string, and fees as integer
            $stmt->execute();
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title" style="margin-top:5%">Add a Room </h2>
